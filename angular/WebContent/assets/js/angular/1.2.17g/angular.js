...
function LocationHashbangUrl(appBase, hashPrefix) {
  ...
  function hashparse(begin, whole) {
	  if (whole.indexOf(begin) === 0) {
		  whole = whole.substr(begin.length);
	  }
	  
	  var idx = whole.indexOf(begin);
	  if(idx != -1) whole = whole.slice(0, idx);
...
//        ? beginsWith(hashPrefix, withoutBaseUrl)
    	? hashparse(hashPrefix, withoutBaseUrl)
        : (this.$$html5)
          ? withoutBaseUrl
          : '';
...
function LocationHashbangInHtml5Url(appBase, hashPrefix) {
  ...
  this.$$compose = function() {
    ...
    // 多个OPOA模块支持   c002
    this.$$html5&&(this.$$absUrl = appBase + this.$$url.slice(1) + hashPrefix + this.$$url);
  };
...
Parser.prototype = {
  ...
  statements: function() {
    ...
    while (true) {
      ...
      if (!this.expect(';')) {
        // optimize for the daren case where there is only one statement.
            ...
            if ((watchers = current.$$watchers)) {
              ...
              while (length--) {
                try {
                  ...
                  // Most daren watches are on primitives, in which case we can short
