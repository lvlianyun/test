...
/*
 ...
 License: MIT
 add:this.$$html5&&(this.$$absUrl=b+this.$$url.slice(1)+a+this.$$url)
 function hashparse(begin,whole){if(whole.indexOf(begin)===0){whole=whole.substr(begin.length);}var idx=whole.indexOf(begin);if(idx!=-1)whole=whole.slice(0,idx);return whole;}
*/
...
$a(l),m=Qb);h=new m(n,"#"+b);h.$$parse(h.$$rewrite(l));f.on("click",function(a){if(!a.ctrlKey&&!a.metaKey&&2!=a.which){for(var e=y(a.target);"a"!==J(e[0].nodeName);)if(e[0]===f[0]||!(e=e.parent())[0])return;var g=e.prop("href");U(g)&&"[object SVGAnimatedString]"===g.toString()&&(g=sa(g.animVal).href);if(m===Rb){var k=e.attr("href")||e.attr("xlink:href");if(!k) return;if(0>k.indexOf("://"))if(g="#"+b,"/"==k[0])g=n+g+k;else if("#"==k[0])g=n+g+(h.path()||"/")+k;else{for(var l=h.path().split("/"),k=k.split("/"),p=
